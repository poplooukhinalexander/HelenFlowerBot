...
WORKDIR /src
COPY ["HelenFlowerBotApp.csproj", "HelenFlowerBotApp/"]
RUN dotnet restore "HelenFlowerBotApp/HelenFlowerBotApp.csproj"

WORKDIR "/src/HelenFlowerBotApp"
COPY . .
RUN dotnet build "HelenFlowerBotApp.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "HelenFlowerBotApp.csproj" \
...
Execute unit tests
```
cd "HelenFlowerBotApp/test/HelenFlowerBotApp.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "HelenFlowerBotApp/src/HelenFlowerBotApp"
